...
6. **Install Requirements**
   - Install dependencies from `requirements.txt`:
     ...
     ```
     In ai agent for connecting use this to import dpendencies 
     They are the dependencies needed to run the simple AI agent with LLMs using Anthropic's Claude model (and optionally OpenAI as a fallback).
      pip install python-dotenv langchain langchain-anthropic anthropic pydantic
      # if using OpenAI fallback:
      pip install openai langchain  # or the required OpenAI langchain extras
